...
object CustomersSql {
  ...
  private val ColumnsWithAddress =
    customerId ~ timestamp ~ firstName ~ lastName ~ regionId ~ townId ~ address ~ date ~ phone ~ region ~ town
  ...
  val decCustomer: Decoder[Customer] = Columns.map {
  ...
  val decCustomerWithAddress: Decoder[CustomerWithAddress] = ColumnsWithAddress.map {
    case id ~ createdAt ~ firstName ~ lastName ~ regionId ~ townId ~ address ~ birthday ~ phone ~ region ~ town =>
      CustomerWithAddress(
        id,
        createdAt,
        firstName,
...
FROM customers
        ...
        INNER JOIN towns ON customers.town_id = towns.id
         """
    baseQuery(Void)
      .whereAndOpt(searchFilter(filters): _*)
  }
